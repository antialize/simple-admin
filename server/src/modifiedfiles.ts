...
import {
    ACTION,
    type IModifiedFilesList,
    type IModifiedFilesResolve,
    type IModifiedFilesScan,
    type IObjectChanged,
    type ModifiedFile,
} from "./shared/actions";
...
export class ModifiedFiles {
    ...
    async scan(client?: WebClient, act?: IModifiedFilesScan) {
        ...
        for (const row of 
            await serverRs.get_deployed_file_like(rs)
            ) {
            ...
            getOrInsert(objects, row.host, () => []).push({
                path,
                type: +row.type,
                host: row.host,
                data,
        ...
        const promises: Promise<{ host: number; content: { path: string; data: string }[] }>[] = [];
        for (const [hostId, objs] of objects) {
            ...
            promises.push(
                new Promise((accept, reject) => {
                    const args: string[] = [];
                    for (const o of objs) args.push(o.path);
                    const script = `
...
`;
    ...
    async list(client: WebClient, act: IModifiedFilesList) {
        client.sendMessage({
            ...
            scanning: this.scanning,
            lastScanTime: this.lastScan,
            changed: this.modifiedFiles.filter((f) => {
                const p = this.props.get(f.id);
                return p && !p.dead;
            }),
            removed: [],
