...
class Docker {
    ...
    async listImageTags(
        ...
        try {
            const [tags, pinned] = await serverRs.listImageTags(rs, +new Date() / 1000 - maxRemovedAge);
            res.tags = tags;
    ...
    async listDeploymentHistory(client: WebClient, act: IDockerListDeploymentHistory) {
        ...
        try {
            const hashes: string[] = [];
            for (const row of await serverRs.getDockerDeployments(rs,
                act.host,
                act.name,
            )) {
                hashes.push(row.hash);
    ...
    async imageSetPin(client: WebClient, act: IDockerImageSetPin) {
        let changed = await serverRs.imageSetPin(rs, act.id, act.pin);
