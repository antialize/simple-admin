...
export async function changeObject(id: number, object: IObject2<any> | null, author: string) : Promise<IV> {
    if (id < 0) 
        return await serverRs.insertObject(rs, db.nextObjectId++, 1, object, author);
    let version = await serverRs.getMaxVersionAndUnsetNewest(rs, id) + 1;
    if (object ) 
        return await serverRs.insertObject(rs, id, version, object, author);
    else
        return {id, version};
}
...
export async function getHostContentByName(hostname: string) : Promise<{
    ...
    let row = await serverRs.getObjectByNameAndType(hostname, hostId);
    if (row === null)
        return null;
    return {
        id: row.id,
...
export async function getHostVariables(id: number): Promise<null | [Host, { [key: string]: string }]> {
