...
const Type = observer(function Type({typeId: myType, id}:{typeId:number, id:number}) {
    ...
    for (const ct of (type && type.content) || []) {
        ...
        case TypePropType.text:
            rows.push(<InformationListRow key={ct.name} name={ct.title}>
                    <Tooltip title={ct.description || ""}>
                        <TextField variant="standard" value={v==undefined?ct.default:v} fullWidth={!!ct.lines && ct.lines > 0} style={{width: 400}} multiline={!!ct.lines && ct.lines > 1} rows={ct.lines || 1} onChange={(e) => setProp(ct.name, e.target.value)}/>
                    </Tooltip>
                </InformationListRow>);
            break;
        case TypePropType.number:
            rows.push(<InformationListRow key={ct.name} name={ct.title}>
                    <Tooltip title={ct.description}>
                        <TextField variant="standard" value={v==undefined?""+ct.default:""+v} onChange={(e) => setProp(ct.name, +e.target.value)}/>
                    </Tooltip>
                </InformationListRow>);
            break;
        case TypePropType.choice:
            rows.push(
                <InformationListRow key={ct.name} name={ct.title}>
                    <Tooltip title="ct.description">
                        <Select variant="standard" value={v==undefined?ct.default:v} onChange={(e) => setProp(ct.name, e.target.value)}>
    ...
    return (
        <div>
            <InformationList key={id + "_" + (current.version || 0)}>
                <InformationListRow name="Name"><TextField key="name" variant="standard" value={current.name} onChange={(e) => {current.name = e.target.value; obj.touched = true;}} /></InformationListRow>
                <InformationListRow name="Comment"><TextField key="comment" variant="standard" fullWidth multiline value={current.comment} onChange={(e) => {current.comment = e.target.value; obj.touched = true;}}/></InformationListRow>
                ...
                {type.hasContains?<InformationListRow name={type.containsName || "Contains"} long={true}><ObjectSelector filter={(type,_)=>(type != hostId && type != typeId && type != rootId)} selected={c.contains?c.contains:[]} setSelected={(sel:number[]) => {setProp("contains",sel)}}/></InformationListRow>:null}
                {type.hasDepends?<InformationListRow name="Depends on" long={true}><ObjectSelector filter={(type, _) => (type != hostId && type != typeId && type != rootId)} selected={c.depends ? c.depends : []} setSelected={(sel: number[]) => { setProp("depends", sel) }}/></InformationListRow>:null}
                {type.hasSudoOn?<InformationListRow name="Sudo on" long={true}><ObjectSelector filter={(type, _) => (type == hostId)} selected={c.sudoOn ? c.sudoOn : []} setSelected={(sel: number[]) => { setProp("sudoOn", sel) }} /></InformationListRow>:null}
               </InformationList>
            {extra}
        </div>);
});
