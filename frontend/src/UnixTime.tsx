...
class UnixTime extends React.Component<{time:number}, {delta: number}> {
    ...
    render() {
        ...
        if (delta < 60*60*72) {
            ...
            let s = (de - m*60).toFixed(0);
            if (y != 0) return <span title={title}>{y}y {d}d ago</span>;
            if (d != 0) return <span title={title}>{d}d {h}h ago</span>;
            if (h != 0) return <span title={title}>{h}h {m}m ago</span>;
            if (m != 0) return <span title={title}>{m}m {s}s ago</span>;
            return <span title={title}>{Math.round(delta*1000)}ms ago</span>;
        }

        const now = new Date();
        if (now.getFullYear() != d.getFullYear()) {
            return <span title={title}>{months[d.getMonth()]} {d.getFullYear()}</span>;
        }
        return <span title={title}>{months[d.getMonth()]} {d.getDate()}</span>;
    }
};
