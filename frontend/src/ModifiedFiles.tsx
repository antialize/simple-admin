...
import { IModifiedFilePage, PAGE_TYPE } from './shared/state';
...
export const ModifiedFileRevolver = observer(function ModifiedFileRevolver(props: {id: number}) {
    ...
    const contentV = content=== null?current:content;
    ...
    return <div className="modified_container">
        <div style={{gridArea: "head"}}>
            ...
            <Switch checked={patch2} onChange={(e)=>setPatch2(e.target.checked)} />
            &nbsp;&nbsp;
            <Button variant="contained" disabled={current == contentV} onClick={()=>setContent(current)}>Reset</Button>
            &nbsp;&nbsp;
            <Button disabled={!patched} variant="contained" onClick={()=>setContent(patched || "")}>Apply patch</Button>
            &nbsp;&nbsp;
            <Button variant="contained" color="secondary" onClick={()=>s.revert(props.id)}>Revert changes on host</Button>
            &nbsp;&nbsp;
            <Button variant="contained" color="primary" disabled={current == content} onClick={()=>s.save(props.id, contentV)}>{s.saveTime?"Wait "+s.saveTime:"Save changes"}</Button>
        </div>
        {
            patch2
            ? <div style={{gridArea: "lt / lb"}}> <Editor lang={"diff"} fixedLang={true} title="Diff" data={patch} readOnly={true}/> </div>
            : <>
                <div className="modified_half" style={{gridArea: "lt"}}><Editor lang={lang} setLang={(lang)=>setLang(lang)} fixedLang={false} title="Deployed" data={o.deployed} readOnly={true}/></div>
                <div className="modified_half" style={{gridArea: "lb"}}><Editor lang={lang} setLang={(lang)=>setLang(lang)} fixedLang={false} title="Current" data={o.actual || ""} readOnly={true}/></div>
            </>
        }
        <div style={{gridArea: "right"}}>
            <Editor lang={lang} setLang={(lang)=>setLang(lang)} fixedLang={false} title="Deployed" data={contentV} setData={(d)=>setContent(d)}/>
        </div>
        </div>;
});
...
export const ModifiedFiles = observer(function ModifiedFiles() {
    ...
    return <Box title="Modified Files" expanded={true} collapsable={false}>
            {s.scanning
                ?<div><CircularProgress /><Span className={"classes.scan"} sx={{
                    marginLeft: "20px",
                    color: "text.primary",
                    fontSize: "120%"
                }}>Scanning</Span></div>
