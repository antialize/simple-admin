...
impl<'a, M> Visitor<'a, M> {
    fn template(&mut self, name: &str, v: &str, deployment_title: &str) -> String {
        let t = match mustache::compile_str(v) {
            Ok(v) => v,
            Err(e) => {
        ...
        match t.render_data_to_string(&self.variables) {
            Ok(v) => v,
            Err(e) => {
                self.errors.push(format!(
                    "Template error in {} of {}: {:?}",
    ...
    fn add_variabels(&mut self, obj: &'a Object) {
        for (key, value) in obj.content.variables_iter() {
            let v = self.template(key, value, &obj.name);
            if let Err(e) = self.variables.add_str(key, v) {
                self.errors.push(format!(
    ...
    fn visit_content(
                ...
                match item {
                    ...
                    crate::type_types::ITypeProp::Bool(IBoolTypeProp {
                        ...
                        if let Some(variable) = variable {
                            if !variable.is_empty() {
                                self.variables.add_bool(variable, v)?;
                            }
                        }
                    ...
                    crate::type_types::ITypeProp::Text(ITextTypeProp {
                        ...
                        let v = obj_content
                            ...
                            .unwrap_or(default);
                        let v = if *template {
                            self.template(name, v, deployment_title)
                        } else {
                            v.to_string()
                        };
                        if let Some(variable) = variable {
                            if !variable.is_empty() {
                                self.variables.add_str(variable, &v)?;
                            }
                        }
                        if deploy_title.unwrap_or_default() {
                            deployment_title = self.string_arena.alloc_str(access, &v);
                        }
                        content.insert(name.to_string(), Value::String(v));
                    }
                    ...
                    crate::type_types::ITypeProp::Document(IDocumentTypeProp {
                        ...
                        if let Some(variable) = variable {
                            if !variable.is_empty() {
                                self.variables.add_str(variable, v.clone())?;
                            }
                        }
                        content.insert(name.to_string(), Value::String(v));
                    }
        ...
        let script = r#type
            .script
            .as_deref()
            .map(|v| self.template("script", v, deployment_title));
    ...
    fn visit_object(
        ...
        let obj = self.objects.get(&id).context("Missing object")?;
        let type_id = obj.r#type;
        let type_content = self
            .types
            .get(&type_id)
            .with_context(|| format!("Missing type {}", type_id))?;
        // It is important to not return until pop is called
        self.variables.push();
        if type_content.has_variables.unwrap_or_default() {
            self.add_variabels(obj);
        }
        if let Some(nv) = &type_content.name_variable {
            if let Err(e) = self.variables.add_str(nv.as_str(), &obj.name) {
                self.errors
                    .push(format!("Failed to add varible {}: {:?}", nv, e));
            }
        }
        let mut res = self.visit_content(access, &obj.name, &obj.content, type_content);
        self.variables.pop()?;
        ...
        if let Ok(res) = &mut res {
            if type_content.has_sudo_on.unwrap_or_default() {
                ...
                res.content.insert("sudoOn".to_string(), Value::Bool(so));
            }
        }
        res
    }
    ...
    fn visit_trigger(
        ...
        let v = self.visit_content(access, "trigger", values, type_content)?;
    ...
    fn visit_triggers(
        ...
        for (id, values) in obj.triggers_iter() {
            match self.visit_trigger(access, id, values) {
                Ok(v) => {
    ...
    fn handle_contains(
        ...
        for child_id in contains.contains_iter() {
            match self.visit(access, child_id, path, prefix) {
                Ok((n, s)) => {
    ...
    fn visit(
        ...
        let mut v = self.visit_object(access, id).with_context(|| {
            format!(
        ...
        if type_content.has_triggers.unwrap_or_default() {
            self.visit_triggers(access, &obj.content, node)
        }
        self.visit_triggers(access, &type_obj.content, node);

        {
            path.push(id);
            let pushed_prefix = if self.variables.has_vars() {
                prefix.push(id);
                true
            } else {
                false
            ...
            if type_content.has_contains.unwrap_or_default() {
                self.handle_contains(access, &obj.content, node, sentinal, path, prefix);
            }
            self.handle_contains(access, &type_obj.content, node, sentinal, path, prefix);
    ...
    fn visit_top(&mut self, access: &mut OCellAccess<M>, id: i64) -> Option<NodePair<'a, M>> {
        ...
        let c = match self.visit(access, id, &mut path, &mut prefix) {
            Ok(v) => Some(v),
            Err(e) => {
...
async fn setup_deployment_host<'a, M>(
    ...
    visitor.variables.add_str("nodename", &host_object.name)?;
    ...
    while let Some(id) = to_visit.pop() {
        ...
        match obj.r#type {
            ...
            HOST_VARIABLE_ID => {
                visitor.add_variabels(obj);
            }
            _ => {}
        }
    }
    visitor.add_variabels(host_object);
    ...
    for (node, sentinal) in visitor.nodes.values() {
        match deploy_id {
            ...
            Some(deploy_id) => {
                ...
                if access.ro(sentinal).base().id == deploy_id || type_id == deploy_id {
    ...
    while let Some(v) = pq.pop() {
        ...
        let type_content = types.get(type_id).context("Missing type")?;
        ...
        if let Some(v) = old_content.remove(*name) {
            if !redeploy {
                ...
                o.prev_content = Some(content.content);
    ...
    if host_full {
        ...
        for (name, v) in values {
            host_deployment_objects.push(IDeploymentObject {
                ...
                prev_script: None,
                next_content: None,
                prev_content: Some(v.content.content),
...
async fn setup_deployment_inner<'a, M>(
    ...
    if let Some(root) = objects.get(&ROOT_INSTANCE_ID) {
        visitor.add_variabels(root);
    }
        ...
        if object.host != cur_host {
            ...
            for row in res {
                ...
                host_objects
                    .entry(row.r#type)
                    .or_default()
                    .insert(row.name, Value::Object(c.content));
            }
        }
