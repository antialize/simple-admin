        ...
        try:
            ...
            if res == True:
                ...
                await output_queue.put(
                    {"type": "failure", "id": id, "failure_type": "unknown"}
                )
...
async def run_instant(obj, output_queue):
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", encoding="utf-8", suffix=obj["name"]
    ) as f:
        f.write(obj["content"])
        f.flush()
        try:
...
async def script_output_handler(id, reader, src, output_queue, type):
    ...
    while True:
        ...
        await output_queue.put(
            {"type": "data", "id": id, "data": data, "source": src, "eof": eof}
        )
        if eof:
            break
    ...
    with open(path, "rb") as f:
        while True:
            ...
            if not data:
                break
            await output_queue.put(
                {"type": "data", "id": id, "data": base64.b64encode(data).decode()}
            )
    return True
...
async def client():
    ...
    try:
        ...
        reader, writer = await asyncio.open_connection(
            config["server_host"], config["server_port"], ssl=sc, limit=1024 * 1024
        )
        ...
        while True:
            try:
                package = await asyncio.wait_for(
                    reader.readuntil(b"\36"), timeout=1200.0
                )
            except asyncio.IncompleteReadError:
                break
            if not package:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", help="path to config file", default="/etc/simpleadmin_client.json"
    )
    parser.add_argument(
        "--hostname", help="the host name to report (default will be taken `hostname`)"
    )
