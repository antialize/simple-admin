...
impl<'de> serde::Deserialize<'de> for ITypeProp {
    // TODO(jakobt) error handeling
    fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        use serde_json::Value;
        let value = Value::deserialize(d)?;
        Ok(match value.get("type").and_then(Value::as_u64).unwrap() {
            0 => ITypeProp::None,
            1 => ITypeProp::Bool(IBoolTypeProp::deserialize(value).unwrap()),
            2 => ITypeProp::Text(ITextTypeProp::deserialize(value).unwrap()),
            3 => ITypeProp::Password(IPasswordTypeProp::deserialize(value).unwrap()),
            4 => ITypeProp::Document(IDocumentTypeProp::deserialize(value).unwrap()),
            5 => ITypeProp::Choice(IChoiceTypeProp::deserialize(value).unwrap()),
            6 => ITypeProp::TypeContent(ITypeContentTypeProp::deserialize(value).unwrap()),
            7 => ITypeProp::Number(INumberTypeProp::deserialize(value).unwrap()),
