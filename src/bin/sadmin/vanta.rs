...
pub async fn setup(config: Config) -> Result<()> {
    ...
    // Invoke the privileged install step via sudo. The secret is passed over
    // stdin rather than as an argument so it doesn't leak via `ps`.
    ...
    if let Some(mut stdin) = child.stdin.take() {
        stdin
            ...
            .context("Failed to write secret to vanta-install-service stdin")?;
    }
    let status = child
        .wait()
        .context("Failed to wait for sudo sadmin vanta-install-service")?;
...
pub fn install_service(args: InstallServiceArgs) -> Result<()> {
    ...
    // Read the secret from stdin rather than a command-line argument so it
    // doesn't leak via `ps`.
...
[Service]
Type=oneshot
ExecStart={exe} vanta-scan-and-report
"#
    );
...
/// Print the list of registered machines and their last scan status.
...
/// Remove a registered machine from the server.
...
/// Run a single compliance scan and, if the result changed since the last
...
/// Vanta host configuration stored in /etc/simpleadmin/vanta.conf.
...
/// Read the VantaConf from a file.
