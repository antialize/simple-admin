...
pub struct Docker {
    ...
    ca: TMutex<Option<(String, String)>>,
}

impl Docker {
    pub async fn ensure_ca_key_crt(&self, db: &Db) -> Result<(String, String)> {
        let mut ca = self.ca.lock().await;
        if let Some(ca) = ca.as_ref() {
            Ok(ca.clone())
        } else {
            let key = db.get(
                "SELECT `value` FROM `kvp` WHERE `key` = 'ca_key'",
                (),
                |v| v.get(0),
            )?;
            let key = match key {
            ...
            let crt = db.get(
                "SELECT `value` FROM `kvp` WHERE `key` = 'ca_crt'",
                (),
                |v| v.get(0),
            )?;
            let crt = match crt {
                Some(v) => v,
                None => {
                    info!("Generating ca crt");
                    let crt = generate_ca_crt(&key).await?;
                    db.insert(
                        "REPLACE INTO kvp (key,value) VALUES (?, ?)",
                        ("ca_crt", &crt),
                    )?;
