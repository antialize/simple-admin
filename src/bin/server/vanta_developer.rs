...
pub async fn scan_handler(
    ...
    // the machine was on (privacy: see vanta_plan.md).
...
#[derive(Serialize, Debug)]
#[serde(rename_all = "camelCase")]
struct VantaDeveloperResources {
    resource_id: String,
    resources: Vec<VantaDeveloperResource>,
}
...
pub async fn push_developer_machines(config: &Config, db: &sqlx::SqlitePool) -> Result<()> {
    ...
    let payload = VantaDeveloperResources {
        resource_id: resource_id.clone(),
        resources,
    };

    let push_req = client
        .put("https://api.vanta.com/v1/resources/CustomResource")
        .bearer_auth(access_token)
        .json(&payload)
        .build()
        .context("Failed building push request")?;

    let push_resp = client
        .execute(push_req)
        .await
        .context("Failed executing push request")?;

    check_vanta_sync(push_resp, "syncing developer machines").await?;

    info!(
        "Successfully synced {} developer machines to Vanta",
