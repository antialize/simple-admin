...
pub async fn get_auth(db: &Db, host: Option<&str>, sid: Option<&str>) -> Result<AuthStatus> {
    ...
    if let Some((user,_)) = sid.split_once(":") {
        ...
        if content.sessions.map(|v| v.split(',').any(|v| v == sid)).unwrap_or_default() {
            Ok(AuthStatus{
                auth: true,
        ...
        let row = db.get("SELECT `pwd`, `otp`, `user`, `host` FROM `sessions` WHERE `sid`=?", (sid,), 
            |r| Ok(Row{pwd: r.get("pwd")?, otp: r.get("otp")?, user: r.get("user")?, host:r.get("host")?}))?;
        ...
        let otp_expiration: u64 = if &user == "docker_client" {
            60 * 60 } else {64 * 24 * 60 * 60}; //otp time out after 2 months

        let now = std::time::SystemTime::now();
        let now = now
            .duration_since(std::time::UNIX_EPOCH)
            .context("Bad unix time")?.as_secs();
        let pwd = row.pwd.map(|v| v+ pwd_expiration > now).unwrap_or_default();
        let otp = row.otp.map(|v| v + u64::max(otp_expiration, pwd_expiration) > now).unwrap_or_default();
        ...
        Ok(AuthStatus{
            ...
            docker_pull: pwd && otp && (content.admin || content.docker_deploy ||  content.docker_pull),
            docker_push: pwd && otp && (content.admin || content.docker_deploy ||  content.docker_push),
