...
pub fn main() -> Result<()> {
    ...
    for v in &exports {
        ...
        let mut current_export: Option<String> = None;
        for line in v.lines() {
            ...
            // Continuation of a multiline export type
            if let Some(ref mut acc) = current_export {
                acc.push(' ');
                acc.push_str(line.trim());
                if line.trim_end().ends_with("};") || line.trim_end().ends_with(';') {
                    let complete = current_export.take().unwrap();
            ...
            if let Some(l) = line.strip_prefix("export type ") {
                ...
                exported.insert(exp);
                // Check if type is complete on this line
                if line.trim_end().ends_with("};") || line.trim_end().ends_with(';') {
                    let line = line
                        .replace(" bigint", " number")
                        .replace("<bigint", "<number")
                    ...
                    // Start accumulating a multiline type
                    current_export = Some(line.to_string());
                }
                continue;
            }
    ...
    let mut cmd = std::process::Command::new("./node_modules/.bin/biome")
        .arg("format")
        .arg("--stdin-file-path")
        .arg("src/shared_types.ts")
